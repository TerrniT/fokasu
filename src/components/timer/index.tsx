      ...
      <div className={"relative mb-8"}>
        <svg width={200} height={200}>
          <g transform={`rotate(-90 ${"100 100"})`}>
            ...
            <Circle
              color={isTimerStart ? isBreak ? "stroke-orange-200" : "stroke-primary" : "stroke-primary/20"}
        ...
        <div className="z-20 absolute top-0 left-0 right-0 bottom-0 flex flex-col gap-y-2 items-center justify-center">
            <div className="text-md">
              {!isBreak ? (
                <LightingIcon size={23} className={cn(isTimerStart ? "text-yellow-500 scale-105 animate-pulse" : "text-yellow-300/20")} />
              ) : (
                <CoffeeIcon className={cn(isTimerStart ? "text-zinc-300 scale-105 animate-pulse" : "text-zinc-300/20")} size={23} />
              )}
            </div>
